...
$(MACOS_PROJ_PATH): $(BUILD_DEPS) $(MACOS_USER_DATA_PATH)/WorkspaceSettings.xcsettings
	...
	SCHEME_NAME=npm-test SCHEME_TYPE=npm BUILDABLE_NAME=mbgl-node.node BLUEPRINT_NAME=mbgl-node NPM_ARGUMENT="test" platform/macos/scripts/create_scheme.sh
	SCHEME_NAME=npm-run-test-suite SCHEME_TYPE=npm BUILDABLE_NAME=mbgl-node.node BLUEPRINT_NAME=mbgl-node NPM_ARGUMENT="run test-suite" platform/macos/scripts/create_scheme.sh
